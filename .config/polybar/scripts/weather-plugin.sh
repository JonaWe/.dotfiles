#!/bin/bash
#
# This file was taken form: https://github.com/GuidoFe/weather-polybar-plugin
# I only made some small adjustments
...
APIKEY=`cat ~/.config/polybar/scripts/openweathermap.key`
CITY_NAME=`cat ~/.config/polybar/scripts/openweathermap.city`
COUNTRY_CODE='DE'
...
# separator settings
COLOR_SEPARATOR="#424452"
SEPARATOR_SYMBOL=""


COLOR_SEPARATOR_BEGIN="%{F$COLOR_SEPARATOR}"
COLOR_SEPARATOR_END="%{F-}"
...
function setIcons {
    ...
    if [ "$DISPLAY_WIND" = "yes" ] && [ `echo "$WINDFORCE >= $MIN_WIND" |bc -l` -eq 1 ]; then
        ...
        WIND="$WIND $COLOR_SEPARATOR_BEGIN$SEPARATOR_SYMBOL$COLOR_SEPARATOR_END"
    fi
...
function outputCompact {
    OUTPUT="$WIND %{T$WEATHER_FONT_CODE}%{F$ICON_COLOR}$ICON %{F-}%{T-} $ERR_MSG$COLOR_TEXT_BEGIN$DESCRIPTION$COLOR_TEXT_END$COLOR_SEPARATOR_BEGIN$SEPARATOR_SYMBOL$COLOR_SEPARATOR_END $TEMP"
